...
export function getConfig(env: AppEnv) {
  return {
    chainId: Number(withFallback(env.VITE_CONFIG_CHAIN_ID, 1337)),
    chainName: withFallback(env.VITE_CONFIG_CHAIN_NAME, "MegaETH"),
    chainNativeCurrency: {
      name: withFallback(env.VITE_CONFIG_CHAIN_NATIVE_CURRENCY_NAME, "MegaETH"),
      symbol: withFallback(env.VITE_CONFIG_CHAIN_NATIVE_CURRENCY_SYMBOL, "ETH"),
      decimals: Number(
    ...
    chainGifUrl: withFallback(env.VITE_CONFIG_CHAIN_GIF_URL, "/megaeth.gif"),
    chainGifAlt: withFallback(env.VITE_CONFIG_CHAIN_GIF_ALT, "Fast MegaETH"),
    heroText: withFallback(
      env.VITE_CONFIG_CHAIN_HERO_TEXT,
      "Move your assets to MegaETH faster than ever!"
    ),

    appTitle: withFallback(env.VITE_CONFIG_APP_TITLE, "MegaETH Fast Bridge"),
    ...
    nexusSupportedChain: Number(
      withFallback(env.VITE_CONFIG_NEXUS_SUPPORTED_CHAIN, 1337)
    ),
