...
const FastBridge: React.FC<FastBridgeProps> = ({
  ...
  React.useEffect(() => {
    ...
    const shouldShow =
      isCompleted && hasData && !hasShownSuccessToastRef.current;
    ...
    if (shouldShow) {
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (
          ...
          explorerUrlRef.current &&
          typeof explorerUrlRef.current === "string" &&
          explorerUrlRef.current.trim() !== ""
        ) {
          ...
          <div className="flex flex-col gap-2 w-full">
            ...
            <div className="flex flex-col gap-1.5 text-sm text-muted-foreground">
              ...
              <div>
                ...
                {formatAmount(amountSpent)} {tokenSymbol}
              </div>
              <div>
              ...
              <div>
                <span className="font-medium">Total Fees:</span> {totalFees}{" "}
                {tokenSymbol}
              </div>
              {explorerUrl ? (
  ...
  const isTransactionInProgress = React.useMemo(() => {
    ...
    const hasCompletionStep = steps.some((s) => {
      const stepType = s.step?.type as unknown as string;
      return (
        s.completed &&
        (stepType === "INTENT_FULFILLED" ||
      ...
      <CardContent className="flex flex-col gap-y-4 w-full">
        <ChainSelect
          ...
          handleSelect={(chain) => {
            ...
            if (!isCurrentTokenSupported) {
              ...
              if (newChainTokens && newChainTokens.length > 0) {
                ...
                newToken =
                  ...
                  (newChainTokens[0]?.symbol as SUPPORTED_TOKENS);
              } else {
            ...
            setInputs((prev) => ({
              ...prev,
              chain,
              token: newToken as SUPPORTED_TOKENS,
            }));
          }}
          label={
            <>
        ...
        {(() => {
          ...
          const exceedsBalance =
            amount > Number.parseFloat(adjustedBalance || "0");
          ...
          return (
            (shouldShowLoading || intentIncomplete) && (
            ...
            return (
              <div className="text-center py-4">
                <p className="text-base text-red-600">
                  Maximum bridge limit exceeded. You can bridge up to {limit}{" "}
                  {filteredUnifiedBalance.symbol} at a time.
                </p>
              </div>
            );
          }
        ...
        {!intent?.intent &&
          (!inputs?.chain ||
            !inputs?.token ||
            !inputs?.amount ||
            Number.parseFloat(inputs?.amount || "0") <= 0 ||
            !inputs?.recipient) && (
            <div className="text-center py-4">
