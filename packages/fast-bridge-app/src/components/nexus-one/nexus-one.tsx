...
const isNativeAddress = (address?: string | null) => {
...
const normalizeStoredHistoryEntry = (
  ...
  return {
    ...
    id: entry.id,
    mode: entry.mode,
    // Older clients inferred refunds from step names; that is not SDK evidence.
    status: entry.status === "refund-initiated" ? "failed" : entry.status,
...
const getFailureMessageForProgressStep = (
  ...
  if (
    ...
    return mode === "send"
      ? "Send failed. Check transaction status."
      : mode === "deposit"
...
function SwapReceiptPanel({
  ...
  const failureHeadline = isDeposit
    ? "Deposit failed"
    : isRecipientTransfer
      ? "Send failed"
      : "Swap failed";
  const failureDescription = isFailed
    ? getReceiptErrorMessage(entry)
    : undefined;
...
function NexusOneInner({
  ...
  const [txError, setRawTxError] = useState<string | null>(null);
  const setTxError = useCallback((message: string | null) => {
    setRawTxError(message ? getUserFacingError(message) : null);
  }, []);
  ...
  const [destinationBalance, setDestinationBalance] =
    useState<TokenOptionBalances | null>(null);
    ...
    return {
      ...
      ...toTokenOptionBalances(
        matchedBreakdown ?? {
          ...
          balanceInFiat: Number.isFinite(sourceValue) ? sourceValue : 0,
        }
      ),
      chainId: source.chain.id,
      chainName: getShortChainName(
  ...
  const getTokenUsdRateFromBalances = (
    ...
    for (const asset of swapBalance ?? []) {
      for (const breakdown of asset.breakdown ?? []) {
        ...
        const balance = parseFiatNumber(breakdown.usableBalance);
  ...
  const getExactOutDestinationBalanceCoverage = ({
    ...
    const balanceAmount =
      parseFiatNumber(destinationBalance?.balance) ??
        ...
        for (const asset of swapBalance ?? []) {
          for (const breakdown of asset.breakdown ?? []) {
            ...
            userTokens.push({
              ...
              name: bSymbol,
              symbol: bSymbol,
              ...toTokenOptionBalances(breakdown),
            });
          }
        }
    ...
    for (const asset of swapBalance ?? []) {
      for (const breakdown of asset.breakdown ?? []) {
        ...
        balance = balance.plus(
          parseFiatNumber(breakdown.usableBalance) ?? new Decimal(0)
        );
      }
    }
  ...
  const getHeldDestinationTokenOption = () => {
    ...
    for (const asset of swapBalance ?? []) {
      for (const breakdown of asset.breakdown ?? []) {
        ...
        const balanceAmount = parseFiatNumber(breakdown.usableBalance);
  ...
  const getExactOutTotalSourceBalanceUsd = (): Decimal => {
    ...
    return getSwapUsableBalanceUsd();
  };
  ...
  const buildInsufficientSourcesIssue = (error: unknown): SwapQuoteIssue => {
    ...
    return {
      ...
      message: getUserFacingError(
        error,
        "There isn't enough usable source balance. Reduce the amount or choose another asset."
      ),
    };
  };
  ...
  const buildSourceTokenSnapshotMap = useCallback(
    (balances: UserAsset[] | null | undefined) => {
      ...
      for (const asset of balances ?? []) {
        for (const breakdown of asset.breakdown ?? []) {
          const chainId = breakdown.chain?.id;
          const rawAddress = breakdown.contractAddress ?? "";
          const contractAddress =
            rawAddress ||
            (isNativeAddress(rawAddress) ? zeroAddress : rawAddress);
          const symbol = breakdown.symbol ?? asset.symbol;

          if (
            !chainId ||
            !symbol ||
            (!contractAddress && !isNativeAddress(contractAddress)) ||
            !isSwapSupportedBySdkChainList(
              chainId,
              swapSupportedChainsAndTokens
            )
          ...
          if (chainId && symbol) {
            snapshots.set(`${chainId}:${symbol.toUpperCase()}`, snapshot);
          }
          ...
          const nativeAlias = getNativeAddressAlias(contractAddress);
          if (chainId && nativeAlias) {
            snapshots.set(`${chainId}:${nativeAlias.toLowerCase()}`, snapshot);
          }
        }
      }
      ...
      const updateToken = (token: SwapTokenOption): SwapTokenOption => {
        ...
        if (token.isUnified) {
          ...
          return {
            ...token,
            ...preservedAmounts,
            ...sumTokenOptionBalances(sourceTokens),
            sourceTokens,
          };
        }

        const snapshot =
          snapshots.get(getTokenSelectionKey(token)) ??
          ...
          (token.chainId &&
          token.contractAddress &&
          getNativeAddressAlias(token.contractAddress)
            ? snapshots.get(
                `${token.chainId}:${getNativeAddressAlias(token.contractAddress)!.toLowerCase()}`
              )
            : undefined) ??
          (token.chainId && token.symbol
            ? snapshots.get(`${token.chainId}:${token.symbol.toUpperCase()}`)
            : undefined);

        if (!snapshot) {
          const existingBalanceNum =
            parseFiatNumber(token.totalBalance) ??
            parseFiatNumber(token.balance) ??
            new Decimal(0);
          const hasExistingBalance = existingBalanceNum.gt(0);

          const existingBalance = hasExistingBalance
            ? {
                balance: token.balance ?? "0",
                balanceInFiat: token.balanceInFiat ?? "$0.00",
                totalBalance: token.totalBalance ?? token.balance ?? "0",
                totalBalanceInFiat:
                  token.totalBalanceInFiat ?? token.balanceInFiat ?? "$0.00",
              }
            : toTokenOptionBalances({ balance: "0", balanceInFiat: 0 });
  ...
  const getDestinationBalanceFromSwapBalances = (token?: SwapTokenOption) => {
    ...
    for (const asset of swapBalance ?? []) {
      for (const breakdown of asset.breakdown ?? []) {
        ...
        return toTokenOptionBalances(breakdown);
      }
    }
  ...
  useEffect(() => {
    ...
    setToToken((current) => {
      const next = {
        ...
        totalBalanceInFiat: current?.totalBalanceInFiat,
      };
      if (
        current &&
  ...
  const handleDone = () => {
    ...
    const hasSource = retained.fromTokens.length > 0;
    ...
    setIntentData(null);
    setPredictiveQuote(null);
    setFromTokens(retained.fromTokens);
    setToToken(retained.toToken);
    setSourceSelectionTouched(hasSource);
    setDepositSourceFilter("all");
    setExactOutQuoteSourceModeValue(hasSource ? "selected" : "all");
    sourcePickerDraftModeRef.current = hasSource ? "selected" : "all";
    sourcePickerDraftTouchedRef.current = hasSource;
    lastAutoIntentSourceTokensRef.current = [];
  ...
  const handleEnterPreview = async (options: { background?: boolean } = {}) => {
      ...
      if (resolvedRecipientAddress.endsWith(".eth")) {
        try {
          ...
          setTxError(
            getUserFacingError(
              e,
              "We couldn't look up this ENS name. Check the name and try again."
            )
          );
          return;
        }
    ...
    try {
      ...
      const showFailedProgressThenReceipt = (
        ...
        // A failed deposit/relay step does not establish refund eligibility.
        const autoRefundAvailable = false;
        setFailedProgressStep(failedStep);
        finishCurrentSwapHistoryEntry("failed", {
          error,
          autoRefundAvailable,
          failureDescription: getUserFacingError(
            error,
            TRANSACTION_STATUS_MESSAGE
          ),
  ...
  const reversedTokenSelection = isMultiAssetMode
    ? undefined
    : reverseTokenSelection({
        fromTokens,
        toToken: toTokenWithFetchedBalance ?? toToken,
      });
  ...
  const handleReverseTokens = () => {
    ...
    setFromTokens(
      swapBalance
        ? patchSourceTokensWithBalances(
            reversedTokenSelection.fromTokens,
            swapBalance
          )
        : reversedTokenSelection.fromTokens
    );
    ...
    resetSourcePickerDraft();
    setSourceSelectionRevision((current) => current + 1);
    const destination = reversedTokenSelection.toToken;
    if (destination) {
      const destBalance =
        getDestinationBalanceFromSwapBalances(destination) ??
        (destination.balance
          ? {
              balance: destination.balance,
              balanceInFiat: destination.balanceInFiat ?? "$0.00",
              totalBalance: destination.totalBalance ?? destination.balance,
              totalBalanceInFiat:
                destination.totalBalanceInFiat ??
                destination.balanceInFiat ??
                "$0.00",
            }
          : null);
      setDestinationBalance(destBalance);
      setToToken(
        destBalance ? { ...destination, ...destBalance } : destination
      );
      onReceiveAssetChange?.({
  ...
  const handleDepositPercentSelect = async (pct: number) => {
    ...
    if (pct !== 100) {
      ...
      try {
        ...
        setTxError(
          getUserFacingError(
            error,
            "We couldn't calculate this amount. Please try again."
          )
        );
      }
      return;
    ...
    try {
      ...
      setTxError(
        getUserFacingError(
          error,
          "We couldn't calculate the maximum deposit. Please try again."
        )
      );
    }
  };
  ...
  const totalSwapBalanceUsd = (swapBalance ?? [])
    .reduce(
      (sum, asset) => sum.plus(getTotalBalanceInFiat(asset)),
      new Decimal(0)
    )
    .toDecimalPlaces(2)
    .toFixed();
              ...
              <>
                ...
                {swapStep === "progress" && (
                  <NexusOneProgressScreen
                    ...
                    failureMessage={
                      currentSwapEntry?.error ?? txError ?? undefined
                    }
                    fromAmountUsd={previewFromAmountUsd}
                    fromTokens={fromTokens}
                    intentData={intentData}
          ...
          {activeMode === "swap" &&
            [
              ...
              <>
                <SwapIdleForm
                  amount={amount}
                  canReverseTokens={Boolean(reversedTokenSelection)}
                  defaultRecipientAddress={defaultRecipientAddress}
                  destinationGasFeeUsd={previewDestinationGasFeeUsd}
                  fromTokens={
                    swapType === "exactOut"
                  ...
                  onReverseTokens={handleReverseTokens}
              ...
              <SwapAssetSelector
                ...
                onSelect={(token) => {
                  ...
                  const shouldClearDestination = Boolean(
                    isSameAsDestination || isSwapExactOutDestinationToken(token)
                  );
                  const baseList =
                  ...
                  const preservedAmount =
                    tokenChanged || shouldClearDestination
                      ? ""
                  ...
                  } else if (swapType === "exactIn" || isReceiveEmptyOrZero) {
            ...
            >
              <ReceiveAssetSelector
                ...
                onSelect={(token) => {
                  ...
                  const destBalance =
                    getDestinationBalanceFromSwapBalances(token) ??
                    (token.balance
                      ? {
                  ...
                  const tokenWithBalance = destBalance
                    ? { ...token, ...destBalance }
                    : token;
                  if (
                    activeMode === "send" ||
