...
interface SwapQuoteIssue {
  message: string;
  missingUsd?: string;
  type: "insufficientSources";
}

interface CachedMaxSwapQuote {
  decimals: number;
...
interface PredictiveQuoteBaseline {
...
const getDisplayDestinationSourceRow = (entry: SwapHistoryEntry) => {
  ...
  if (!(entry.toToken && entry.requestedToAmount)) {
    return null;
  }
  ...
  if (
    !(requestedAmount && destinationBalanceAmount) ||
  ...
  return {
    ...
    amount: displayAmount
      ...
      .toFixed(0),
    value: rate
      ? displayAmount.mul(rate).toFixed(0)
...
export function NexusOne({
  ...
  const [_sourceSelectionRevision, setSourceSelectionRevision] = useState(0);
  ...
  const [_transferExplorerUrl, _setTransferExplorerUrl] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    if (historyStorageKeyRef.current === historyStorageKey) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setHistoryNow(Date.now()), 30_000);
  ...
  const getCachedIntentUsdRate = (
    ...
    return rate?.gt(0) ? rate : undefined;
  };
  ...
  const cacheDestinationUsdRateFromIntent = (
    ...
    const cached: CachedIntentUsdRate = {
      ...
      rate: rate.toDecimalPlaces(18).toFixed(0),
      updatedAt: Date.now(),
  ...
  const getUsdRateForSymbol = (symbol?: string) => {
    ...
    const rate = parseFiatNumber(cached?.rate);
    return rate?.gt(0) ? rate : new Decimal(0);
  };
  ...
  const getUsdForTokenAmount = (
    ...
    const rate = getTokenUsdRate(token);
    if (rate.lte(0) || tokenAmount.lte(0)) {
      return new Decimal(0);
    }
  ...
  const getExactOutDestinationBalanceCoverage = ({
    ...
    const externalAmount = producedAmount?.gt(0)
      ? producedAmount
      : new Decimal(0);
    ...
    if (coveredAmount.lte(0)) {
      return null;
    }

    const requestedRate = requestedUsd?.gt(0)
      ? requestedUsd.div(requestedAmount)
      : undefined;
    const producedRate =
      producedUsd?.gt(0) && producedAmount && producedAmount.gt(0)
        ? producedUsd.div(producedAmount)
        : undefined;
    const fallbackRate = getTokenUsdRate(token);
    const usdRate = requestedRate?.gt(0)
      ? requestedRate
      : producedRate?.gt(0)
        ? producedRate
        : fallbackRate.gt(0)
  ...
  const buildDestinationBalanceDisplayToken = (
    ...
    if (!(coverage && token) || coverage.amount.lte(0)) {
      return null;
    }
    ...
    const balanceUsd = coverage.usd
      ? `$${coverage.usd.toDecimalPlaces(2, Decimal.ROUND_DOWN).toFixed(0)}`
  ...
  const getPredictiveDisplayAmount = (
    ...
    return amount.toDecimalPlaces(decimals, Decimal.ROUND_DOWN).toFixed(0);
  };
  ...
  const allocateUnifiedExactInToken = (
    ...
    if (token.userAmountMode === "usd") {
      ...
      for (const source of sortedSources) {
        ...
        allocated.push({
          ...source,
          userAmount: tokenAmount.toFixed(0),
          userAmountMode: "token",
    ...
    for (const source of sortedSources) {
      if (remainingTokenAmount.lte(0)) {
        break;
      }

      const availableTokenAmount = getTokenBalanceAmount(source);
      if (availableTokenAmount.lte(0)) {
        continue;
      }
  ...
  const hasGasForSource = (token: SwapTokenOption) => {
    ...
    if (isNativeTokenAddress(token.contractAddress)) {
      return true;
    }
        ...
        tokens.push({
          ...
          balanceInFiat:
            fiatBalance !== undefined
              ? `$${fiatBalance.toDecimalPlaces(2).toFixed(0)}`
              : "$0.00",
        });
      }
    }
  ...
  const getExactInSourceDeficitUsd = () => {
    ...
    return fromTokens.reduce((sum, token) => {
      ...
      if (token.userAmountMode === "usd") {
        ...
        if (!availableUsd || requestedAmount.lte(availableUsd)) {
          return sum;
        }
  ...
  const buildInsufficientSourcesIssue = (error: unknown): SwapQuoteIssue => {
    ...
    let missingUsd = exactInSourceDeficitUsd?.gt(0)
    ...
    if (missingUsd?.gt(0)) {
      ...
      return {
        type: "insufficientSources",
        missingUsd: missingUsd.toDecimalPlaces(2).toFixed(0),
  ...
  const _formatReadableTokenAmount = (rawAmount: bigint, decimals: number) =>
  ...
  const _trimDecimalString = (value: string) =>
  ...
  const markSwapExecutionStarted = () => {
    if (currentSwapStartedAtRef.current > 0) {
      return;
    }
    const now = Date.now();
  ...
  const cachePredictiveBaselineFromIntent = (intent: SwapIntentData) => {
    ...
    if (!destinationAmount || destinationAmount.lte(0)) {
      return;
    }

    const destinationUsdRate = destinationValue?.gt(0)
      ? destinationValue.div(destinationAmount)
      : getUsdRateForSymbol(intent.destination?.token?.symbol);
    if (destinationUsdRate.lte(0)) {
      return;
    }
    ...
    const baseline: PredictiveQuoteBaseline = {
      destinationUsdRate: destinationUsdRate.toDecimalPlaces(18).toFixed(0),
      updatedAt: Date.now(),
    };
    ...
    const resolvedDestinationValue = destinationValue?.gt(0)
      ? destinationValue
  ...
  const applySwapIntent = useCallback(
        ...
        const bridgeTotal =
          typeof bridgeFees === "string"
            ...
            : (parseFiatNumber(bridgeFeeData?.total) ??
              (bridgeComponentsTotal?.gt(0)
                ? bridgeComponentsTotal
                : undefined));

        if (bridgeTotal !== undefined) {
          setIntentFeeUsd(
            bridgeTotal.gt(0) ? bridgeTotal.toDecimalPlaces(6).toFixed(0) : "0"
          );
        } else {
          setIntentFeeUsd(undefined);
        }
    ...
    [
      ...
      cacheDestinationUsdRateFromIntent,
      cachePredictiveBaselineFromIntent,
      parseFiatNumber,
    ]
  );
  ...
  const getDestinationBalanceFromSwapBalances = (token?: SwapTokenOption) => {
    ...
    for (const asset of swapBalance ?? []) {
      for (const breakdown of asset.breakdown ?? []) {
        if (breakdown.chain?.id !== token.chainId) {
          continue;
        }
        ...
        return `${balance.toDecimalPlaces(6).toFixed(0)} ${token.symbol}`;
      }
    }
  ...
  const resolvePrefillToken = useCallback(
    ...
    [supportedChainsAndTokens, swapBalance, isNativeTokenAddress]
  );
  ...
  useEffect(() => {
    ...
    if (!(sourcePrefill || destinationPrefill)) {
      return;
    }
    ...
    if (appliedTokenPrefillRef.current === prefillKey) {
      return;
    }
    ...
    if (sourcePrefill && !sourceToken) {
      return;
    }
    if (destinationPrefill && !destinationToken) {
      return;
    }
  ...
  useEffect(() => {
    if (config.prefill?.amount) {
      setAmount(config.prefill.amount);
    }
  ...
  useEffect(() => {
    ...
    if (!(balanceToken?.chainId && ownerAddress)) {
      return;
    }
    ...
    toToken,
    formatReadableTokenBalanceAmount,
    toTokenFromOpportunity,
  ]);
  ...
  }, [activeMode, selectedOpportunity, toTokenFromOpportunity]);
  ...
  useEffect(() => {
    ...
    if (!config.opportunities || config.opportunities.length <= 1) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (getFiatValue(1, toToken.symbol) > 0) {
      return;
    }

    let cancelled = false;
    void resolveTokenUsdRate(toToken.symbol).catch((error: unknown) => {
      if (!cancelled) {
  ...
  }, [getFiatValue, resolveTokenUsdRate, toToken?.symbol]);
  ...
  const _maxBalance = currentAsset?.balance
  ...
  const depositUsdDecimal =
    depositAmountMode === "usd"
      ...
      : depositTokenAmountForQuote
        ...
        : new Decimal(0);
  const depositUsdDisplay = depositUsdDecimal.toDecimalPlaces(2).toFixed(0);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const destinationRate = cachedDestinationRate?.gt(0)
        ? cachedDestinationRate
        : await resolveUsdRateForToken(toToken);

      if (cancelled || runId !== predictiveQuoteRunRef.current) {
        return;
      }
      ...
      const predictedDestinationAmount = cachedAmountPerSourceUsd?.gt(0)
        ? sourceUsd.mul(cachedAmountPerSourceUsd)
        : sourceUsd
            .mul(BASIS_POINTS - PREDICTIVE_EXACT_IN_DISCOUNT_BPS)
            .div(BASIS_POINTS)
            .div(destinationRate);
      const predictedDestinationUsd = cachedAmountPerSourceUsd?.gt(0)
      ...
      setPredictiveQuote({
        ...
        toUsd: predictedDestinationUsd.toDecimalPlaces(6).toFixed(0),
      });
    })();
    ...
    toToken?.decimals,
    toToken?.symbol,
    getPredictiveQuoteCacheKey,
    parseFiatNumber,
    resolveUsdRateForToken,
    toToken,
    getPredictiveDisplayAmount,
    getPredictiveExactInSourceTokens,
  ]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      setPredictiveQuote({
        ...
        toUsd: destinationUsd.toDecimalPlaces(6).toFixed(0),
      });
    })();
    ...
    resolveUsdRateForToken,
    toToken,
    buildPredictiveExactOutSources,
  ]);

  const defaultDepositSourceTokens = useMemo<SwapTokenOption[]>(() => {
    if (activeMode !== "deposit" || !swapBalance) {
      return [];
    }
  ...
  }, [activeMode, swapBalance, hasMinimumSourceUsdBalance]);
  ...
  useEffect(() => {
    ...
    if (lockedDestinationSourceTokens.length === 0) {
      return;
    }
    if (activeMode === "deposit" && !sourceSelectionTouched) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (
      !(toToken && depositTokenAmountForQuote) ||
  ...
  const handleConnectWallet = async () => {
    if (walletActionPending || nexusLoading || isWalletConnectPending) {
      return;
    }
  ...
  const handleEnterPreview = async (options: { background?: boolean } = {}) => {
    ...
    if (
      !background &&
      (activeMode === "deposit" || activeMode === "send") &&
      (!(intentData && swapIntentRef.current) ||
        swapIntentRef.current.runId !== swapRunIdRef.current ||
        (intentData.sources ?? []).length === 0)
    ) {
    ...
    if ((!background && activeMode === "send") || hasCustomSwapRecipient) {
      ...
      } else if (!isAddress(resolvedRecipientAddress)) {
  ...
  useEffect(() => {
    if (activeMode !== "swap" || swapStep !== "idle" || !nexusSDK) {
      return;
    }
    ...
    clearPendingSwapIntent,
    handleEnterPreview,
    hasReadyExactInSwapInput,
  ]);

  useEffect(() => {
    if (activeMode !== "deposit" || swapStep !== "idle" || !nexusSDK) {
      return;
    }
  ...
  useEffect(() => {
    if (activeMode !== "send" || swapStep !== "idle" || !nexusSDK) {
      return;
    }
  ...
  const refreshActiveSwapIntent = useCallback(async () => {
    ...
    try {
      ...
      if (!updated || swapRunIdRef.current !== runId) {
        return;
      }
  ...
  const handleDepositPercentSelect = async (pct: number) => {
    ...
    if (pct !== 100) {
      ...
      const nextAmount =
        depositAmountMode === "usd"
          ? usdAmount.toDecimalPlaces(2, Decimal.ROUND_DOWN).toFixed(0)
  ...
  const quoteCtaLabel = (fallback: string) => {
    if (needsWalletConnection) {
      return walletCtaLabel;
    }
    if (exactOutInsufficientSourceIssue) {
    ...
    if (quoteRefreshing) {
      return "Intent fetching...";
    }
    if (isQuoteUnavailableForAutoSourceFlow) {
      return "Quote unavailable";
    }
    if (!hasPositiveRootAmount) {
      return "Enter amount";
    }
    return fallback;
  };
  ...
  const previewDestinationAmount =
    ...
    previewExactOutDestinationAmount?.gt(0)
      ? previewExactOutDestinationAmount
          ...
          .toFixed(0)
      : intentToAmount;
  const previewFromAmountUsd = previewSourceUsdNumber?.gt(0)
    ? previewSourceUsdNumber.toDecimalPlaces(6).toFixed(0)
    : undefined;
  const previewToAmountUsd = previewDestinationUsdNumber?.gt(0)
    ? previewDestinationUsdNumber.toDecimalPlaces(6).toFixed(0)
    : undefined;
        ...
        >
          <div className="flex items-center gap-x-2">
            {canGoBack && (
              ...
              >
                <ArrowLeft className="h-5 w-5" style={{ color: "#161615" }} />
              </button>
            )}
            <div
              style={{
            ...
            {isTitleCentered() &&
              ...
              swapStep === "idle" &&
              selectedOpportunity && (
                <div className="pointer-events-auto relative ml-2 flex items-center">
                  <button
                    className="flex items-center gap-1 rounded-[4px] py-1 pr-1.5 pl-2 transition-colors hover:bg-black/5"
                    onClick={() => {
                      clearPendingSwapIntent();
                      setSelectedOpportunity(undefined);
                      setToToken(undefined);
            ...
            swapStep !== "enter-recipient" && (
              <>
                ...
                {(swapStep === "success" || swapStep === "failed") &&
                  currentSwapEntry && (
                    <div className="fade-in slide-in-from-bottom-4 w-full animate-in duration-500">
                      <SwapReceiptPanel
