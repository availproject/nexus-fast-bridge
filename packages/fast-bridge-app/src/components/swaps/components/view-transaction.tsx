...
const MultiSourceBreakdown = ({
  ...
  return (
    <div className="flex items-center w-full justify-between">
      ...
      <StackedTokenIcons maxDisplay={4} size="lg" sources={stackedSources} />
    </div>
  );
};
...
const ViewTransaction: FC<ViewTransactionProps> = ({
  ...
  const exactOutSelectedTotalUsd = useMemo(() => {
    if (!shouldShowExactOutSourceSelection) {
      return 0;
    }
    if (!(exactOutSourceOptions.length && exactOutSelectedKeys.length)) {
      return 0;
    }
