...
const useSwaps = ({
  ...
  const exactOutSourceOptions = useMemo<ExactOutSourceOption[]>(() => {
    ...
    sortedSources.forEach((source, index) => {
      ...
      for (const option of options) {
        if (option.chainId !== source.chainID) {
          continue;
        }
        const optionComparableAddress = toComparableSdkAddress(
          option.tokenAddress
        );
        if (optionComparableAddress !== sourceComparableAddress) {
          continue;
        }
    ...
    return options.sort((a, b) => {
      ...
      if (
        Number.isFinite(aBalance) &&
        Number.isFinite(bBalance) &&
        aBalance !== bBalance
      ) {
  ...
  const selectedSourceInputKey = useMemo(() => {
    ...
    if (!sourceToken || typeof sourceChainId !== "number") {
      return null;
    }
  ...
  const isExactOutAllSelected = useMemo(() => {
    if (exactOutAllSourceKeys.length === 0) {
      return true;
    }
    return (
  ...
  const toggleExactOutSource = useCallback(
    (key: string) => {
      setExactOutSelectedKeys((prev) => {
        ...
        if (next.length === 0) {
          return prev ?? allKeys; // keep at least 1
        }
        if (next.length === allKeys.length) {
          return null; // back to default "all"
        }
        return next;
      });
    },
  ...
  const exactOutFromSources = useMemo<SwapSource[] | undefined>(() => {
    ...
    if (exactOutSourceOptions.length === 0) {
      return undefined;
    }
      ...
      onEvent: (event) => {
        ...
        if (event.type === "plan_progress") {
          ...
          if (completed) {
            ...
            if (
              event.stepType === "destination_swap" &&
              (event as any).txHash
            ) {
              dispatch({
                type: "setExplorerUrls",
  ...
  const handleExactOutSwap = async (runId: number) => {
    ...
    if (
      !(nexusSDK && areExactOutInputsValid && toToken && toAmount && toChainID)
    ) {
      return;
    }
  ...
  const runSwap = async (runId: number) => {
    if (!(nexusSDK && areInputsValid && swapBalance)) {
      return;
    }
  ...
  const continueSwap = useCallback(async () => {
    ...
    try {
      ...
      const nextIntent = swapIntent.current;
      if (!nextIntent || nextIntent === previousIntent) {
        return;
      }
      if ((nextIntent.intent.sources?.length ?? 0) === 0) {
        return;
      }
