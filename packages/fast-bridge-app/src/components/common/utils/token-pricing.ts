...
const DEFAULT_USD_PEGGED_TOKEN_SYMBOL_SET = new Set<string>(
...
export async function fetchCoinGeckoUsdRate(
  ...
  const exactSymbolMatches = (searchBody?.coins ?? [])
    .filter(
      (coin) =>
        normalizeTokenSymbol(coin.symbol ?? "") === normalized && coin.id
    )
    .sort((a, b) => {
...
export async function getTokenPriceUSD(
  ...
  if (DEFAULT_USD_PEGGED_TOKEN_SYMBOL_SET.has(normalized)) {
...
export function useTokenPrice(tokenSymbol: string | undefined): number {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getTokenPriceUSD(tokenSymbol)
      .then((nextPrice) => {
        if (!cancelled) {
          setPrice(nextPrice ?? 0);
        }
      })
      .catch(() => {
        if (!cancelled) {
