...
import { SUPPORTED_CHAINS, type UserAsset } from "@fastbridge/nexus-adapter";
...
import { type FastBridgeState } from "@/components/fast-bridge/hooks/useBridge";
import { SHORT_CHAIN_NAME } from "@/components/common";
    ...
    <div className="flex flex-col gap-y-2 pb-2 w-full">
      <div className="w-full flex sm:flex-row flex-col border border-border rounded-lg gap-y-2">
        <Input
          ...
          onChange={(event) => {
            let next = event.target.value.replaceAll(/[^0-9.]/g, "");
