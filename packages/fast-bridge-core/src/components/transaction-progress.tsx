...
import { type BridgeStepType, type SwapStepType } from "@fastbridge/nexus-adapter";
...
const TransactionProgress: FC<TransactionProgressProps> = ({
  ...
  const completedSteps = Array.isArray(steps)
    ? steps.reduce((acc, step) => acc + (step?.completed ? 1 : 0), 0)
    : 0;
  ...
  const { effectiveSteps, currentIndex } = useMemo(() => {
    ...
    const current = displaySteps.findIndex((step) => !step.completed);
    return { effectiveSteps: displaySteps, currentIndex: current };
  }, [completed, percent, timer]);
