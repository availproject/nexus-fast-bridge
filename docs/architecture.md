...
- Single-mode direction reversal swaps both tokens and chains (or moves the single selected asset to the opposite side) and keeps the source input's amount and token/USD mode when applicable. It clears the old receive amount, cached USD value, percentage and quote, then validates any retained input against the new source's usable balance. A retained source amount drives an exact-input quote. Multi-source or unified selections cannot be reversed.
